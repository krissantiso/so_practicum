...
void Cmd_open (char * pz[]) {   //AQUI ES DONDE TE TIENES QUE PONER A MIRAR
    int i,df, mode=0;
    if (pz[0]==NULL) {
        ListOpenFiles(F);
        return;
    ...
    for (i=0; pz[i]!=NULL; i++) {
        if (!strcmp(pz[i], "cr")) mode |= O_CREAT;
        else if (!strcmp(pz[i], "ex")) mode |= O_EXCL;
        else if (!strcmp(pz[i], "ro")) mode |= O_RDONLY;
        else if (!strcmp(pz[i], "wo")) mode |= O_WRONLY;
        else if (!strcmp(pz[i], "rw")) mode |= O_RDWR;
        else if (!strcmp(pz[i], "ap")) mode |= O_APPEND;
        else if (!strcmp(pz[i], "tr")) mode |= O_TRUNC;
        else break;
    }
    if ((df=open(pz[0],mode,0777))==-1) {
        perror("Imposible abrir fichero");
    } else {
        AddFile(df,pz[0],pz[1],&F);
        printf ("Entry %d has been added to the file list\n",df);
    }
}
